...
class TestRiskAnalysis(TestCase):
    ...
    def test_foo(self):
        ...
        risk_analysis_service = RiskAnalysisService(product_status_builder)

        risk_analysis_rules_factory = RiskAnalysisRulesFactory(PersonRules(), AssetRules())
        rules_list = risk_analysis_rules_factory.build_rules_list()

        risk_analysis_service.apply_rules_on(self.risk_analysis, rules_list)
        risk_analysis_service.execute_risk_analysis(risk_score=self.risk_analysis.risk_profile.get_risk_score())
